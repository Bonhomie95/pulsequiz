...
1. **Deploy the server.** Already live at `https://pulsequiz-api.onrender.com` (Render, free plan, deploys the `store-ready-upgrades` branch, MongoDB Atlas). `eas.json` points the app there. Before launch move it to a paid plan — the free plan sleeps after 15 minutes idle, so the first request takes ~50s — and add the remaining secrets in Render → Environment: `FACEBOOK_APP_ID`, `FACEBOOK_APP_SECRET`, `APPLE_*`, `GOOGLE_SERVICE_ACCOUNT_JSON`, `NOWPAYMENTS_API_KEY`, `NOWPAYMENTS_IPN_URL`. Without them Facebook/Apple sign-in, Play webhooks and payouts stay off. (See RUNBOOK) and make sure `https://<api-host>/terms` etc. load. If you run a marketing site at `pulsequiz.app`, serve or proxy the same five paths and set `EXPO_PUBLIC_WEB_URL`.
...
3. **AdMob:** rewarded-ad verification is now enforced (`ADMOB_SSV_ENABLED=1`), so **no coins are credited until you set the SSV callback** `https://<api-host>/api/webhooks/admob/ssv` on each rewarded unit. iOS currently runs Google's **test** ad units, which earn nothing and hide the "watch an ad" option on iOS; replace the three `EXPO_PUBLIC_ADMOB_*_ID_IOS` values in `eas.json` before release. Also create **iOS** ad units under the same publisher as the iOS app id (`ca-app-pub-7561090708148190~…`) and add them to `eas.json` as `EXPO_PUBLIC_ADMOB_*_ID_IOS`. The current units belong to the Android publisher and are scoped to Android. Publish a GDPR/US-states consent message (Privacy & messaging). Set the rewarded SSV callback and `ADMOB_REWARDED_AD_UNIT_IDS` on the server.
4. **App Store Connect**
