...
export async function getHomeSummary(req: AuthRequest, res: Response) {
  ...
  const [wallet, streakDoc, lastSession, weeklySnapshot, monthlySnapshot, allSnapshot] = await Promise.all([
    CoinWallet.findOne({ userId }).lean(),
    Streak.findOne({ userId }).lean(),
    QuizSession.findOne({ userId }).sort({ createdAt: -1 }).select('category score createdAt').lean(),
  ...
  try {
    ...
    const pool = await PrizePool.findOne({ type: 'weekly', periodLabel }).lean();
  ...
  return res.json({
    ...
    lastQuiz: lastSession
      ? { category: lastSession.category, score: lastSession.score, playedAt: lastSession.createdAt }
...
export async function getReadyPlayers(req: AuthRequest, res: Response) {
  ...
  const recentUserIds = recentSessions.map((s) => s._id.toString()).filter((id) => id !== userId);
